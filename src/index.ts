...
async function main() {
  ...
  const audit = new FileAuditLogger(config.audit.output || '/tmp/adg-audit.log');
  ...
  if (mcpOnly) {
    ...
    await startMCPServer({ config, credentialStore, connectors, auditLogger: audit });
    return;
  }
  ...
  try {
    ...
    if (mcpMode) {
      ...
      const connectors = buildConnectorRegistry(config, credentialStore);
      startMCPServer({ config, credentialStore, connectors, auditLogger: audit });
    }
  } catch (err) {
  ...
  for (const source of config.sources) {
    ...
    if (source.type === 'postgres') {
      registry.set(source.name, {
        execute: async (_, query) => pgConn.execute(source.name, query.text, connectorConfig),
      });
